...
public class TmnlOpenUserInfoLogSink extends AbstractSink implements Configurable {
    private static final Log logger = LogFactory.getLog(BossAgentLogSink.class);

    private static final String topic = "tmnl_oper_user_info_log_topic";
...
//        props.put("batch.num.messages", "1000000");//默认值：200
        ...
        logger.info("TmnlOpenUserInfoLogSink初始化完成.");
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            logger.error("Flume TmnlOpenUserInfoLogSinkException:", e);
            tx.rollback();
            return Status.BACKOFF;
        } finally {
            tx.close();
