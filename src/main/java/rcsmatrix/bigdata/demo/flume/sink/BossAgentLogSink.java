...
public class BossAgentLogSink extends AbstractSink implements Configurable {
    private static final Log logger = LogFactory.getLog(BossAgentLogSink.class);

    private static final String topic = "boss_agent_log_topic";
...
//        props.put("batch.num.messages", "1000000");//默认值：200
        ...
        logger.info("BossAgentLogSink初始化完成.");
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            logger.error("Flume BossAgentLogSinkException:", e);
            tx.rollback();
            return Status.BACKOFF;
        } finally {
            tx.close();
