...
object BoosAgentLogETL {
  def main(args: Array[String]) {
    //var args = Array("20","email")ktr
    if (args.length!=2){
      System.err.println("Usage: BoosAgentLogETL <seconds> <event>")
      System.exit(1)
    }
    val event = args(1)
    val conf = new SparkConf().setAppName("BoosAgentLogETL")
    ...
    .foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        .map(line => {
          val e = EtlTool.getBoosAgentLog(line._2)//抽取
        ...
        //正常数据
        val re2 = r.filter(_._1.length>0)
        re2.reduceByKey((a,b) => {("")}).map(_._1).collect()
        for(ab <- re2){
          println("ab:"+ab)
        }
