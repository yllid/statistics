...
object RegisterLogETL {
  def main(args: Array[String]) {
    //var args = Array("20","email")ktr
    if (args.length!=2){
      System.err.println("Usage: RegisterLogETL <seconds> <event>")
      System.exit(1)
    }
    val event = args(1)
    val conf = new SparkConf().setAppName("RegisterLogETL")
    ...
    .foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        .map(line => {
          val e = EtlTool.getRegisterLog(line._2)//抽取
