...
object UserDevChangeLogETL {
  def main(args: Array[String]) {
    //var args = Array("20","email")ktr
    if (args.length!=2){
      System.err.println("Usage: UserDevChangeLogETL <seconds> <event>")
      System.exit(1)
    }
    val event = args(1)
    val conf = new SparkConf().setAppName("UserDevChangeLogETL")
    ...
    .foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        .map(line => {
          val e = EtlTool.getUserDevChangeLog(line._2)//抽取
